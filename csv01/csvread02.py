...
def main():
    with open('superbirthday.csv', mode='r') as csv_file:
        ...
        for row in csv_reader:
            ...
            # use open to write a new file with just the regular birthdays
            with open('regularbirthday.csv',  mode = 'w') as new_file:
                #line_count2 = 0
                if line_count == 0:
                    new_file.write('Column names are {} and {}'.format(row["name"], row["birthday month"]))
                
                new_file.write('\t{} was born in {}.'.format(row["name"], row["birthday month"]))
                #line_count2 += 1
