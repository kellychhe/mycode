...
"""Learning splicing and for loops"""

# dictionary for the heros
classinfo = {
    "all": [
        {
            "name": "Aaron",
...
def main():
    ...
    # find name kelly and print name
    name = classinfo["all"][15]["name"]
    print(name)

    # find name kelly and show output from fill in the blank
    print(f"My name is {name} and my super power is {classinfo['all'][15]['super power']}.")

    # looping through all of classinfo["all"] and printing a summarizing statement
    for hero in classinfo["all"]:
        name = hero["name"]
        skill = hero["skill level"]
        animal = hero["spirit animal"]
        power = hero["super power"]
        
        # conditionally use a and an
        if skill[0].lower() in ["a", "e", "i", "o", "u"]:
            n = "n"
        else:
            n = ""

        print(f"\n{name}, a{n} {skill} {animal} of a programmer, possesses a {power} factor for the moonlighting as a superhero!")
